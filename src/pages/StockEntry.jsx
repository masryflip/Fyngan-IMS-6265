...
const { FiSave, FiPackage, FiMapPin, FiInfo, FiRefreshCw, FiGrid, FiChevronDown, FiChevronUp, FiAlertTriangle, FiCheckCircle, FiAlertCircle, FiFilter } = FiIcons;

export default function StockEntry() {
  const { items, categories, locations, stockLevels, isLoading, updateStockLevel, formatQuantity } = useInventory();
  ...
  const getLocationItems = (locationId) => {
    ...
    return items.filter(item => {
      ...
      return stockLevels.some(stock => stock.item_id === item.id && stock.location_id === locationId);
    });
  };
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Stock Entry</h1>
        <p className="text-sm sm:text-base text-gray-600">Update stock levels organized by category for easy management</p>
      </div>

      {/* Info Banner */}
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4 mb-4 sm:mb-6">
        <div className="flex items-start space-x-3">
          <SafeIcon icon={FiInfo} className="text-blue-600 text-lg mt-0.5 flex-shrink-0" />
          <div>
      ...
      <div className="bg-white rounded-lg shadow-md">
        <div className="p-4 sm:p-6 border-b">
          <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
            <SafeIcon icon={FiMapPin} className="text-coffee-600 text-xl flex-shrink-0" />
            <div className="flex-1">
              ...
              <select
                ...
                className="w-full px-3 py-2 text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-coffee-500"
              >
          ...
          <div className="p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 space-y-3 sm:space-y-0">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="w-full sm:w-auto flex items-center justify-center space-x-2 bg-coffee-600 text-white px-4 py-3 sm:py-2 rounded-md hover:bg-coffee-700 transition-colors disabled:opacity-50 text-base sm:text-sm"
              >
            ...
            {locationItems.length === 0 ? (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 sm:p-6 text-center">
                <SafeIcon icon={FiAlertCircle} className="text-yellow-600 text-3xl sm:text-4xl mb-4 mx-auto" />
                ...
                <div className="flex flex-col sm:flex-row justify-center space-y-2 sm:space-y-0 sm:space-x-4">
                  <a
                    href="#/items"
                    className="inline-flex items-center justify-center px-4 py-2 bg-yellow-600 text-white rounded-md hover:bg-yellow-700"
                  >
                    <SafeIcon icon={FiPackage} className="mr-2" />
                    Manage Item Assignments
                  </a>
                </div>
                <div className="mt-4 p-3 sm:p-4 bg-yellow-100 rounded-lg text-left max-w-md mx-auto">
                  <h4 className="font-medium text-yellow-900 mb-2">How to assign items to locations:</h4>
                  <ol className="text-xs sm:text-sm text-yellow-800 space-y-1">
                  ...
                  {categorizedItems.length > 0 ? (
                    categorizedItems.map((category, categoryIndex) => (
                      ...
                      >
                        {/* Category Header */}
                        <div
                          className={`p-3 sm:p-4 cursor-pointer ${getCategoryHeaderColor(category)} border-b`}
                          onClick={() => toggleCategory(category.id)}
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-3">
                              <SafeIcon icon={FiGrid} className="text-lg flex-shrink-0" />
                            ...
                            <div className="flex items-center space-x-2 flex-shrink-0">
                              ...
                              {category.outOfStock > 0 && (
                                <span className="px-1.5 py-0.5 sm:px-2 sm:py-1 bg-red-600 text-white text-xs font-bold rounded-full">
                                  {category.outOfStock} OUT
                                </span>
                              )}
                              {category.critical > 0 && (
                                <span className="px-1.5 py-0.5 sm:px-2 sm:py-1 bg-orange-600 text-white text-xs font-bold rounded-full">
                                  {category.critical} CRIT
                                </span>
                              )}
                              {category.low > 0 && (
                                <span className="px-1.5 py-0.5 sm:px-2 sm:py-1 bg-yellow-600 text-white text-xs font-bold rounded-full">
                                  {category.low} LOW
                                </span>
                              )}
                              <SafeIcon icon={collapsedCategories[category.id] ? FiChevronDown : FiChevronUp} className="text-xl" />
                            </div>
                          </div>
                        </div>
                            ...
                            <div className="space-y-3">
                              {category.items.map((item, itemIndex) => (
                                <motion.div
                                  ...
                                  className={`flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 border rounded-lg transition-all hover:shadow-md space-y-3 sm:space-y-0 ${
                                ...
                                >
                                  <div className="flex items-start space-x-3 sm:space-x-4 flex-1 min-w-0">
                                    <SafeIcon 
                                      ...
                                      className={`text-lg mt-0.5 flex-shrink-0 ${getStockStatusColor(item.stockStatus)}`} 
                                    />
                                    <div className="flex-1 min-w-0">
                                      <h4 className="font-medium text-gray-900 text-sm sm:text-base truncate">{item.name}</h4>
                                      <div className="flex flex-col space-y-1 text-xs sm:text-sm text-gray-600 mt-1">
                                        <span>
                                  ...
                                  <div className="flex items-center space-x-2 sm:space-x-3 justify-end">
                                    <div className="flex items-center space-x-2">
                                      <input
                                        ...
                                        className="w-20 sm:w-24 px-2 sm:px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-coffee-500 text-center font-medium text-sm sm:text-base"
                                      />
                                      <span className="text-xs sm:text-sm text-gray-500 min-w-[2rem] sm:min-w-[3rem]">{item.unit}</span>
                                    </div>
                                  </div>
                                </motion.div>
                              ))}
